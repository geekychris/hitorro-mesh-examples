#!/usr/bin/env bash
# mTLS variant of mesh-tls-smoke.sh — NATS requires a client cert
# (verify: true), mesh clients present a PKCS12 keystore that chains
# to the same CA. Proves phase-7a keyStore support end-to-end.
#
# Uses its own work directory + port range so it composes cleanly with
# both mesh-smoke.sh (plain) and mesh-tls-smoke.sh (server-side TLS only).
set -euo pipefail
cd "$(dirname "$0")"
source ./env.sh

: "${MESH_MTLS_WORK:=/tmp/hitorro-mesh-mtls-smoke}"
: "${MESH_MTLS_NATS_PORT:=4422}"
: "${MESH_MTLS_DRIVER_PORT:=8285}"
: "${MESH_MTLS_AGENT_US_PORT:=8291}"
: "${MESH_MTLS_AGENT_EU_PORT:=8292}"
: "${MESH_MTLS_STORE_PASSWORD:=changeit}"

MESH_LOGS="$MESH_MTLS_WORK/logs"
MESH_PIDS="$MESH_MTLS_WORK/pids"
CERTS="$MESH_MTLS_WORK/certs"
CONFIG="$MESH_MTLS_WORK/config"
DATA="$MESH_MTLS_WORK/data"
TYPES="$MESH_MTLS_WORK/types"
...
# ---------- 1. CA + server cert + CLIENT cert (both chain to same CA) ----------
echo "[cert] generating CA + server + client certs under $CERTS"
if [ ! -f "$CERTS/ca.pem" ]; then
    ...
    openssl req -new -x509 -key "$CERTS/ca-key.pem" \
                ...
                -subj "/CN=hitorro-mesh-mtls-smoke-ca" 2>/dev/null
fi
...
if [ ! -f "$CERTS/client.pem" ]; then
    openssl genrsa -out "$CERTS/client-key.pem" 2048 2>/dev/null
    openssl req -new -key "$CERTS/client-key.pem" -out "$CERTS/client.csr" \
                -subj "/CN=hitorro-mesh-client" 2>/dev/null
    openssl x509 -req -in "$CERTS/client.csr" \
                 -CA "$CERTS/ca.pem" -CAkey "$CERTS/ca-key.pem" -CAcreateserial \
                 -out "$CERTS/client.pem" -days 3650 2>/dev/null
fi

# PKCS12 truststore for the Java client (contains CA to verify server).
if [ ! -f "$CERTS/truststore.p12" ]; then
    keytool -import -alias hitorro-mesh-ca -file "$CERTS/ca.pem" \
            ...
            -storepass "$MESH_MTLS_STORE_PASSWORD" -noprompt >/dev/null
fi
# PKCS12 keystore for the Java client (client cert + private key). openssl
# builds a proper key+cert bundle; the JVM's PKCS12 reader picks it up as-is.
if [ ! -f "$CERTS/keystore.p12" ]; then
    openssl pkcs12 -export -inkey "$CERTS/client-key.pem" -in "$CERTS/client.pem" \
                   -name hitorro-mesh-client -out "$CERTS/keystore.p12" \
                   -password "pass:$MESH_MTLS_STORE_PASSWORD" 2>/dev/null
fi

# ---------- 2. NATS server config — mTLS (verify: true) ----------
cat > "$CONFIG/nats-mtls.conf" <<EOF
port: $MESH_MTLS_NATS_PORT
tls {
...
# ---------- 4. mesh configs — nats-security with BOTH trust + key stores ----------
cat > "$CONFIG/driver.yml" <<EOF
server: { port: $MESH_MTLS_DRIVER_PORT }
...
hitorro:
  mesh:
    driver:
      transport: nats
      nats-url: tls://localhost:$MESH_MTLS_NATS_PORT
      nats-security:
        tls: true
        trust-store-path: $CERTS/truststore.p12
        trust-store-password: $MESH_MTLS_STORE_PASSWORD
        key-store-path: $CERTS/keystore.p12
        key-store-password: $MESH_MTLS_STORE_PASSWORD
      agent-expiry: 15s
      tables:
        - name: docs
...
for who in us eu; do
    ...
    portvar="MESH_MTLS_AGENT_${upper}_PORT"
    port=${!portvar}
...
_launch nats "$MESH_NATS_BIN" -c "$CONFIG/nats-mtls.conf"
_wait_tcp localhost "$MESH_MTLS_NATS_PORT" nats

_launch driver java -jar "$MESH_DRIVER_JAR" --spring.config.location=file:"$CONFIG/driver.yml"
_wait_tcp localhost "$MESH_MTLS_DRIVER_PORT" driver 60
...
# ---------- 6. verify ----------
echo "[wait] both agents to register (client cert accepted by nats)"
deadline=$(( $(date +%s) + 20 ))
while :; do
    count=$(curl -s "http://localhost:$MESH_MTLS_DRIVER_PORT/mesh/agents" | grep -o '"agentId"' | wc -l | tr -d ' ' || echo 0)
    [ "$count" = "2" ] && break
    [ "$(date +%s)" -ge "$deadline" ] && _mesh_die "only $count/2 agents registered — mTLS handshake failure? check $MESH_LOGS/agent-us.log, $MESH_LOGS/agent-eu.log"
    sleep 0.5
done
echo "[ok] both agents registered — mTLS handshake succeeded end-to-end"

echo "[query] SELECT id, title FROM docs WHERE lang='en'"
resp=$(curl -sS -X POST "http://localhost:$MESH_MTLS_DRIVER_PORT/mesh/queries" \
...
echo "[ok] query returned $rowCount rows across mTLS-secured shuffle"

# Sanity: driver log should show tls=true (auth still 'none' because we
# use cert-based auth, not user/pass/token/creds-file — client identity
# comes from the TLS handshake itself).
